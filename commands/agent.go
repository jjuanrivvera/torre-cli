...
// localGroups are command subtrees that never talk to the Torre API (setup/meta/local).
...
func init() {
	metaRegistrars = append(metaRegistrars, func(_ *deps) *cobra.Command {
		agentCmd := &cobra.Command{
			...
			Long:  "Generate safety configuration for AI agents that drive torre.",
		}
		...
		guard := &cobra.Command{
			...
			Short: "Generate agent-safety config that blocks destructive torre operations",
...
accepted: a line that merely QUOTES a blocked command is denied.`,
			Example: `  torre agent guard --host claude-code
...
func writeClaudeCodeFiles(cmd *cobra.Command, cls classification) error {
	...
	files := map[string]string{
		".claude/hooks/torre-guard.sh":      hook,
		".claude/torre-guard.settings.json": settings,
	}
	...
	fmt.Fprintln(cmd.ErrOrStderr(), "merge .claude/torre-guard.settings.json into .claude/settings.json to activate the hook")
	return nil
}
...
func bashPattern(path string) string { return "Bash(torre " + path + ":*)" }

// mcpToolPattern is the MCP tool name a host gates: torre_<group>_<verb> under the torre
...
func mcpToolPattern(path string) string {
	return "mcp__torre__torre_" + strings.ReplaceAll(path, " ", "_")
}
