// Package output renders Torre API JSON in table/json/yaml/csv/id. One renderer serves
...
// preferredOrder lists Torre fields that should lead a table when present, before the
// rest fall back to alphabetical. Deterministic ordering is required by GOAL.md §11.
var preferredOrder = []string{
	"id", "username", "name", "objective", "tagline", "professionalHeadline",
	"opportunity", "type", "status", "remote", "commitment", "created", "slug",
	"locations", "picture", "verified",
}
