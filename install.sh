#!/bin/sh
# torre installer for macOS and Linux.
#
...
#   TORRE_VERSION=v1.2.3        pin a version (default: the latest release)
...
REPO="jjuanrivvera/torre-cli"
BINARY="torre"
VERSION="${TORRE_VERSION:-}"
...
if [ -z "$VERSION" ]; then
  ...
  [ -n "$VERSION" ] || die "could not determine the latest release; set TORRE_VERSION"
fi
